...
if(isset($_GET['language']) && isset($_GET['year']) && !isset($_GET['submit2']))
{
    /* wikipedia */
...
//    for($i = 0;$i < count($movienamearray);$i++)
    for($i = 0;$i < 10;$i++)
    {
        $url = $base.str_replace(" ","%20",$movienamearray[$i])."%20".$_GET['year']."%20malayalam";
    ...
    echo '<form action="checkbox.php" method="get">';
    ...
    for($i = 0;$i < count($torrentlinkarray);$i++)
    {
        ...
        else
        {
            ...
            $value = "link".$i;
            echo "<tr>"."<td>".$i."</td>"."<td>".$movienamearray[$i]."</td>"."<td>".'<input type="checkbox" name="checkbox[]" value='.$value.'></td>'."</tr>";
        }
    }
    echo "</table>";
...
{
    $torrentstodownload = $_GET['checkbox'];
    echo "submit2 button pressed...";
    print_r($torrentstodownload);

    foreach($torrentstodownload as $key => $value)
        echo '<p>'.$key. " = ".$value .'</p>';
}
?>
